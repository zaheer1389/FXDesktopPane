...
public class FXWindowIcon extends Button {

	private Button btnClose;
	private Label lblName;
	private FXDesktopPane mdiCanvas;
	 ...
	 * *************************** CONSTRUCTOR
	 */
	public FXWindowIcon(ImageView imgLogo, FXDesktopPane mdiCanvas, String name)
			throws Exception {
		super();
		HBox hBox = new HBox();
		...
		btnClose.addEventHandler(MouseEvent.MOUSE_ENTERED,
				e -> btnClose.setEffect(shadowCloseBtn));
		// Removing the shadow when the mouse cursor is off
		btnClose.addEventHandler(
		...
		hBox.getChildren().addAll(
				imgLogo == null ? new ImageView() : new ImageView(
						imgLogo.getImage()), lblName, btnClose);
		setGraphic(hBox);
	}
	...
	private EventHandler<MouseEvent> handleMaximize = new EventHandler<MouseEvent>() {
		...
		public void handle(MouseEvent event) {
			FXWindow win = mdiCanvas.getItemFromMDIContainer(getId());
			if (win != null) {
				win.setVisible(true);
				win.toFront();
				removeIcon();
	...
	private void removeIcon() {
		FXWindowIcon icon = mdiCanvas.getItemFromToolBar(getId());
		if (icon != null) {
