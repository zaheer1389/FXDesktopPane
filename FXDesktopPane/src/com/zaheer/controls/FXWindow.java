...
public class FXWindow extends BorderPane {
	...
	private FXWindow.ResizeMode resizeMode;
	private FXWindow.AlignPosition alignPosition;
	 ...
	 * @throws Exception
	 */
	public FXWindow(String mdiWindowID, ImageView logoImage, String title,
			Node content) {
	 ...
	 */
	public FXWindow(String mdiWindowID, ImageView logoImage, String title,
			Node content, boolean disableResize) {
	 ...
	 */
	public FXWindow(String mdiWindowID, ImageView logoImage, String title,
			Node content, boolean disableResize, boolean maximize) {
		this.disableResize = disableResize;
		init(mdiWindowID, logoImage, title, content);
		if (maximize) {
			centerMdiWindow();
	...
	private void init(String mdiWindowID, ImageView logoImage, String title,
			Node content) {
	...
	private AnchorPane makeTitlePane(String title) {
		...
		if (!disableResize) {
			paneTitle.getChildren().add(
					makeControls(btnMinimize, btnMaximize, btnClose));
	...
	public void minimizeMdiWindow() {
		...
		borderPane.fireEvent(new FXWindowEvent(imgLogo,
				FXWindowEvent.EVENT_MINIMIZED));
	}
	...
	public void maximizeRestoreMdiWindow() {
		...
		if (isMaximized == false) {
			...
			try {
				...
				Logger.getLogger(FXWindow.class.getName()).log(Level.SEVERE,
						null, ex);
			}
	...
	private void moveListener() {
		this.setOnMouseDragged((MouseEvent dragEvent) -> {
			if (!isMaximized) {
				...
				if (resizeMode == resizeMode.NONE) {
					...
					if (borderPane.getWidth() < borderPane.getParent()
							.getLayoutBounds().getWidth()) {// if the panel is
															// not biger then
															// the window: Move
						borderPane.setLayoutX(x);
					}
					if (borderPane.getHeight() < borderPane.getParent()
							.getLayoutBounds().getHeight()) {// if the panel is
																// not biger
																// then the
																// window: Move
					...
					// LEFT AND RIGHT
					if (borderPane.getLayoutX() <= borderPane.getParent()
							.getLayoutX()) {
						borderPane.setLayoutX(borderPane.getParent()
								.getLayoutX());
					} else if ((borderPane.getLayoutX() + borderPane.getWidth()) >= borderPane
					...
					// UP AND DOWN
					if (borderPane.getLayoutY() <= borderPane.getParent()
							.getLayoutX()) {
						borderPane.setLayoutY(borderPane.getParent()
								.getLayoutY());
					} else if ((borderPane.getLayoutY() + borderPane
							.getHeight()) >= borderPane.getParent()
					...
					if (!disableResize) {
						...
						if (RESIZE_RIGHT && !RESIZE_TOP && !RESIZE_BOTTOM) {
							if (dragEvent.getX() <= (getParent()
									.getLayoutBounds().getWidth() - getLayoutX())) {
								setPrefWidth(dragEvent.getX());
							} else {
								setPrefWidth(getParent().getLayoutBounds()
										.getWidth() - getLayoutX());
							}
						} // Only The Bottom Resize
						else if (!RESIZE_RIGHT && !RESIZE_TOP && RESIZE_BOTTOM) {
							if (dragEvent.getY() <= (getParent()
									.getLayoutBounds().getHeight() - getLayoutY())) {
								setPrefHeight(dragEvent.getY());
							} else {
								setPrefHeight(getParent().getLayoutBounds()
										.getHeight() - getLayoutY());
							}
						} // Only The Bottom with Right Resize
						else if ((RESIZE_RIGHT && !RESIZE_TOP && RESIZE_BOTTOM)) {
							if (dragEvent.getX() <= (getParent()
									.getLayoutBounds().getWidth() - getLayoutX())
									&& dragEvent.getY() <= (getParent()
											.getLayoutBounds().getHeight() - getLayoutY())) {
								setPrefWidth(dragEvent.getX());
								setPrefHeight(dragEvent.getY());
							} else {
		...
		onMouseMovedProperty().set(
				(MouseEvent t) -> {
					final double scaleX = localToSceneTransformProperty()
							.getValue().getMxx();
					final double scaleY = localToSceneTransformProperty()
							.getValue().getMyy();
					final double border1 = 5;
					double diffMinX = Math.abs(getBoundsInLocal().getMinX()
							- t.getX());
					double diffMinY = Math.abs(getBoundsInLocal().getMinY()
							- t.getY());
					double diffMaxX = Math.abs(getBoundsInLocal().getMaxX()
							- t.getX());
					double diffMaxY = Math.abs(getBoundsInLocal().getMaxY()
							- t.getY());
	...
	public void placeMdiWindow(AlignPosition alignPosition) {
		Platform.runLater(() -> {
			((FXDesktopPane) this.getParent().getParent()).placeMdiWindow(this,
					alignPosition);
		});
	}
	...
	public void centerMdiWindow() {
		Platform.runLater(() -> {
			((FXDesktopPane) this.getParent().getParent())
					.centerMdiWindow(this);
		});
	}
	...
	public void closeMdiWindow() {
		...
		st.play();
		borderPane
				.fireEvent(new FXWindowEvent(null, FXWindowEvent.EVENT_CLOSED));
		st.setOnFinished((ActionEvent t) -> {

			FXDesktopPane mdiCanvas = (FXDesktopPane) this.getParent()
					.getParent();
			for (int i = 0; i < mdiCanvas.getPaneMDIContainer().getChildren()
					.size(); i++) {
				FXWindow window = (FXWindow) mdiCanvas.getPaneMDIContainer()
						.getChildren().get(i);
	...
	public enum AlignPosition {
		...
		CENTER, CENTER_LEFT, CENTER_RIGHT, TOP_CENTER, TOP_LEFT, TOP_RIGHT, BOTTOM_LEFT, BOTTOM_RIGHT, BOTTOM_CENTER
	}
